...
for r in range(nR):
    spots_assigned = []
    for c in range(nC):
        coords = spots.loc[(spots['round']==r+1) & (spots['channel']==c+1), ['z-coord', 'y-coord', 'x-coord']].to_numpy()
...
if ismip:
    ...
    if len(filenames) > 0:
        ...
        for r, mip in enumerate(mips):
            mip_warped = mip.copy()
            for ch in range(nC):
                mip_warped[ch] = shift(mip[ch], shift=(shifts_allrounds[r][ch][-2], shifts_allrounds[r][ch][-1]))

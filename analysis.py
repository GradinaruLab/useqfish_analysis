...
for filename in filenames[:roundRef]:
    ...
    dapi_cropped = img[0][:, int(size_y/2-shift_window_size):int(size_y/2), int(size_x/2-shift_window_size):int(size_x/2)]
    ...
    dapis_shifted.append(image_warp(dapi_cropped, shift=shifts[0]))
    # dapi_shifted = img_as_ubyte(image_warp(img[0], shift=shifts[0]))
    # dapi_name = filename.split('_')[0]
    # zarr.save(
    #     os.path.join(os.path.join(path, 'result/dapis_shifted'), filename.split('_')[0]), 

...
# %%
adata_endo.obs["leiden_sub"] = adata_endo.obs["leiden_sub"].cat.rename_categories(
    cluster_list
)
...
adata_endo.obs["leiden_new"] = (
    adata_endo.obs["leiden_sub"].map(new_labels).astype("category")
)
...
transcription_rate_scaled = (
    transcription_rate - transcription_rate.min(axis=1)[:, None]
) / transcription_rate.max(axis=1)[:, None]

leiden_sorted_order = ["19", "20", "5", "17", "14", "18", "15", "12", "4", "3", "2"]

leiden_new_order = list(
    np.unique(np.asarray(adata_endo.obs[clustering_method]).astype(np.uint))
)

...
if ismip:
    ...
    spots = []
    last_overlap = (
        np.array(stitching_shape) * np.array(img_size)
        ...
        if w % 2 == 0:
            for h in range(montage_h):
                ...
                if (w == montage_w - 1) & (h < montage_h):
                    spot_notoverlapped = spot.loc[
                        spot["y-coord"] < overlapped_region[1]
                    ]
                if (w < montage_w - 1) & (h == montage_h - 1):
                    spot_notoverlapped = spot.loc[
                        spot["x-coord"] < overlapped_region[0]
                    ]
                if (w < montage_w - 1) & (h < montage_h - 1):
                    spot_notoverlapped = spot.loc[
                        (spot["x-coord"] < overlapped_region[0])
                ...
                spot_notoverlapped["x-coord"] = (
                    spot_notoverlapped["x-coord"]
                    + w * img_size[0]
                    - w * img_size[0] * stitching_overlap / 100
                )
                spot_notoverlapped["y-coord"] = (
                    -spot_notoverlapped["y-coord"]
                    + h * img_size[1]
                    - h * img_size[1] * stitching_overlap / 100
                )
                if w == montage_w - 1:
                    spot_notoverlapped["x-coord"] = (
                        spot_notoverlapped["x-coord"] - last_overlap[0]
                    )
                if h == montage_h - 1:
                    spot_notoverlapped["y-coord"] = (
                        spot_notoverlapped["y-coord"] - last_overlap[1]
                    )

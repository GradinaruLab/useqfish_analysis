...
def subclustering(target_cluster, mother_cluster, daughter_cluster):
    ...
    sc.tl.dendrogram(adata_sub, use_rep='X_pca', groupby=daughter_cluster, linkage_method='ward')
    sc.pl.heatmap(
        adata_sub,
        gene_list_ordered[7:],
    ...
    sc.tl.dendrogram(adata_endo, groupby=daughter_cluster, use_rep='X_pca', linkage_method='ward')
    sc.pl.heatmap(
        adata_endo,
        gene_list_ordered[7:],
        groupby=daughter_cluster,
...
adata_endo = adata_endo[:, ~adata_endo.var_names.isin(['slc30a3'])]
...
# %%
print(adata_endo.to_df().head())

sns.scatterplot(data=adata_endo.to_df(), x='gad1', y='slc17a7')
...
# %%
subclustering(target_cluster=0, mother_cluster='leiden', daughter_cluster='leiden_sub_0')
# %%
subclustering(target_cluster=1, mother_cluster='leiden_sub_0', daughter_cluster='leiden_sub_1')

# %%
subclustering(target_cluster=2, mother_cluster='leiden_sub_1', daughter_cluster='leiden_sub_2')

# %%
subclustering(target_cluster=3, mother_cluster='leiden_sub_2', daughter_cluster='leiden_sub_3')

# # %%
# subclustering(target_cluster=34, mother_cluster='leiden_sub_3', daughter_cluster='leiden_sub_33')

# # %%
# subclustering(target_cluster=40, mother_cluster='leiden_sub_33', daughter_cluster='leiden_sub_40')
...
#     103,                                        # necab1                                         # ppp1r17
#     108,                                        # slc17a7
#     24, 110, 109, 79, 114, 32, 86,              # L2/3/4
#     105,                                        # L4
#     62, 104,                                    # L5
#     112, 101, 85, 82,                           # L6
#     51,                                         # Sp
...
# ]
leiden_idx_reordered_int = [
    57, 15, 9, 5, 0, 59, 53,                  # pan-layer/gene-specific
    51, 50,                                 # L2/3 (51, 50-L4)
    22, 47, 55, 52,                 # L4 (47-L2/3, 22, 55)
    40,                                 # L5
    60, 56, 49,                      # L6 (60-L5, 56-6a, 49-6b)
    39,                                 # sp (ctgf)
    58, 33, 37, 34, 29, 1, 13, 8           # inh
]
poor_idx = ['30']

gene_list_ordered = [
    'PHP.eB', 'CAP-B10', 'PHP.N', 'PHP.Astro', 'PHP.V1', 'PHP.B8',
    'chrna6', 'crym', 'trh', 'necab1', 'hsd11b1', 'ppp1r17', 'foxp2', 
    'slc17a7', 'lamp5', 'cux2', 'calb1', 'rorb','foxp1', 
    'calb2', 'crh',
    'pcp4', 'tpbg','rprm',
    'ctgf',
    'gad1', 'gad2','tac1', 'pvalb','th', 'sst', 'reln', 'vip', 'sncg', 'lgi2'
]
...
# leiden_idx_reordered = list(np.unique(adata_endo.obs[clustering_method]))
n_clusters = np.unique(list(adata_endo.obs[clustering_method])).size

cell_labels_stitched = zarr.load('./expression_matrices/220116/cell_labels_stitched.zarr')
